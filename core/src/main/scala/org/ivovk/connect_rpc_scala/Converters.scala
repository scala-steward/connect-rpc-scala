...
object Converters extends HeadersConv, StatusCodesConv

trait HeadersConv {

  def http4sHeadersToGrpcMetadata(headers: Headers): Metadata = {
  ...
  def grpcMetadataToHttp4sHeaders(metadata: Metadata): Headers = {
...
trait StatusCodesConv {

  // Url: https://connectrpc.com/docs/protocol/#error-codes
  def mapGrpcStatusCodeToHttpStatus(code: io.grpc.Status.Code): org.http4s.Status = code match {
  ...
  def mapGrpcStatusCodeToConnectCode(code: io.grpc.Status.Code): String = code match {
